...
def launch(name, command, cpus=2, mem=1, gpu=False, log=True, qos=None, array=None, depends=None):
  ...
  with open(slurmfile, 'w') as f:
    ...
    command = "singularity exec docker://brandongeren/hcc-ssbmai " + command
    f.write(command)
...
if run_trainer:
  ...
  train_command = "python3 -u ourphillip/phillip/phillip/train.py --load " + args.path
  train_command += " --dump " + ("lo" if args.local else "eth1")
...
if run_agents:
  ...
  common_command = "python3 -u ourphillip/phillip/phillip/run.py --load " + args.path
  if args.disk:
